...
const Update_job_seeker_profile = () => {
	...
	const onSubmit = async(values) => {
		...
		try {
			const { fullName , email , username , gender , phone , address } = values
			const photo = await upload();
			await updateJobSeeker ({ 
				id :jobSeeker?._id,
				UpdateJobSeeker: {fullName , email , username , gender , phone , address , photo}
			}).then((res)=>{
  ...
  return (
    <div className="w-full text-[#333333] bg-white shadow rounded p-4">
		...
		onSubmit={onSubmit}>
			<Form className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-3 justify-start items-start">
