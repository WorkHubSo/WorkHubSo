	...
	const jobComponent = (
		<div className="p-2 w-full bg-white shadow rounded flex flex-col lg:flex-row justify-start items-start gap-5">
			<div className="w-full p-2 lg:w-[75%]">
				<h1 className="text-xl tracking-wider">Job description</h1>
			...
			<div className="w-full lg:w-[35%] p-2 border-[1px] rounded-md">
				<h1 className="text-xl tracking-wider">Job Summary</h1>
				<hr className="w-full border-[1px] mt-4 border-slate-300" />
				<div className="space-y-5 p-3 mt-4">
					<p className='text-base flex flex-row justify-start items-center gap-2'><MdTitle size={20} />  <span className="">Job Title : </span>  {res_job?.jobTitle}</p>
					<p className='text-base flex flex-row justify-start items-center gap-2'><MdOutlineCategory size={20} />  <span className="">Category : </span>  {res_job?.category}</p>
					<p className='text-base flex flex-row justify-start items-center gap-2'><MdLocationOn size={20} />  <span className="">Location : </span>  {res_job?.location}</p>
					<p className='text-base flex flex-row justify-start items-center gap-2'><CgCalendarDates size={20} />  <span className="">Deadline : </span>   {res_job?.deadline?.substring(0, 10)}</p>
					<p className='text-base flex flex-row justify-start items-center gap-2'><BiTime size={20} /> <span className="">Type : </span>   {res_job?.typeEmployement}</p>
					<p className='text-base flex flex-row justify-start items-center gap-2'><MdOutlineAttachMoney size={20} />  <span className="">Salary : </span>  {res_job?.salary}</p>
				</div>
			</div>
		</div>
	)
